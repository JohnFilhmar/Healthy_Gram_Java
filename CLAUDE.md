...
`HealthyGram` is a Java 8 Swing desktop app for a health-food shop: login, point of sale, and inventory, backed by a local MySQL database. Package root is `healthy_gram`.

## Build & run

There is **no Maven or Gradle**. This is a raw Eclipse JDT project (`.project`, `.classpath`) that compiles `src/` into `bin/`. `bin/` is gitignored and untracked; the Eclipse config files (`.project`, `.classpath`, `.settings/`) are tracked on purpose, since they are this project's only build definition.
...
Screen flow: `Login` → on successful `verifyUser`, opens `PointOfSale` and disposes itself. `PointOfSale`'s Logout button reverses it. `Inventory` is currently reachable only via its own `main()`, and nothing navigates to it.
...
`public/` holds `logo1.png`, `logo2.jpg`, `jms.png`. `Login.java` loads two of them by **absolute path** pointing at `C:\Users\olajo\eclipse-workspace\HealthyGram\public\`, which is not this checkout, so the logo and window icon silently fail to render. Prefer classpath or relative loading if you change that code.
